...
import Foreign.Hoppy.Generator.Types (charT, intT, voidT, enumT, bitspaceT, constT, objT, ptrT, refT)
...
c_QMessageLogger =
  ...
  --, test (qtVersion >= [5, 3]) $ mkConstMethod "critical" [enumT e_CategoryFunction, ptrT $ constT charT] voidT
  , just $ mkConstMethod' "critical" "criticalWithQDebug" [] $ objT c_QDebug
  --, test (qtVersion >= [5, 3]) $ mkConstMethod' "critical" "criticalWithQLogging" [refT $ constT $ objT c_QLoggingCategory] $ objT c_QDebug
  --, test (qtVersion >= [5, 3]) $ mkConstMethod' "critical" "criticalWithCategory" [enumT e_CategoryFunction] $ objT c_QDebug
  ...
  --, test (qtVersion >= [5, 3]) $ mkConstMethod' "debug" "debugWithCategory" [enumT e_CategoryFunction, ptrT $ constT charT] voidT
  ...
  --, test (qtVersion >= [5, 5]) $ mkConstMethod' "info" "infoWithCategory" [enumT e_CategoryFunction, ptrT $ constT charT] voidT
  ...
  --, test (qtVersion >= [5, 3]) $ mkConstMethod' "warning" "warningWithCategory" [enumT e_CategoryFunction, ptrT $ constT charT] voidT
